...
<div class="statpal-odds-container statpal-odds-premium-container" data-paginated="true">
    ...
    <?php else: ?>
        ...
        ?>
            <div id="tourn-<?php echo esc_attr($idx); ?>" class="statpal-hr-tournament" style="display: <?php echo $idx === 0 ? 'block' : 'none'; ?>;">
                <div class="statpal-hr-t-header" style="background: #b59243; color: #fff; padding: 12px 15px; display: flex; justify-content: space-between; align-items: center;">
                    <div class="statpal-hr-t-header-left" style="font-size: 12px; font-weight: 700; color: #fff;">
                        <?php echo esc_html($t_name); ?>
                    </div>
                    <div class="statpal-hr-t-header-right" style="font-size: 12px; font-weight: 600; background: rgba(255,255,255,0.2); padding: 4px 10px; border-radius: 20px;">
                                        ...
                                        <div class="statpal-bookmakers-slider-container">
                                            <div class="statpal-bookmakers-slider">
                                                <?php if(empty($all_bookmakers)): ?>
                                                    <div class="statpal-bm-header"><div class="bm-logo-wrap" style="background:transparent;"><span>Odds</span></div></div>
                                                <?php else: ?>
        ...
        <script>
        (function() {
            var tabsContainer = document.querySelector('.statpal-hr-live-container');
            if(!tabsContainer) return;

            tabsContainer.addEventListener('click', function(e) {
                var tab = e.target.closest('.statpal-hr-t-tab');
                if(!tab) return;
                
                var parent = tab.closest('.statpal-hr-live-container');
                parent.querySelectorAll('.statpal-hr-t-tab').forEach(function(t) {
                    t.classList.remove('is-active');
                });
                tab.classList.add('is-active');
                
                var targetId = tab.getAttribute('data-target');
                parent.querySelectorAll('.statpal-hr-tournament').forEach(function(tourn) {
                    if (tourn.id === targetId) {
                        tourn.style.display = 'block';
                    } else {
                        tourn.style.display = 'none';
                    }
                });
            });
        })();
